...
query.prepare("INSERT INTO FAMILLES (ID, MATRICULE, NOM, BESOIN, NBR_MEMBRES, REGION, DON) VALUES (:MATRICULE, :MATRICULE,:NOM,:BESOIN,'1',  :REGION,:DON)");
...
query.prepare("Delete from familles  where ID = :id ");
...
QSqlQueryModel * famille::afficherBesoin(){ QSqlQueryModel *model = new QSqlQueryModel;
  model->setQuery("SELECT * FROM FAMILLES  ORDER BY BESOIN ");
...
QSqlQueryModel * famille::afficherNBR(){
  ...
  model->setQuery("SELECT * FROM FAMILLES ORDER BY NBR_MEMBRES");
...
QSqlQueryModel * famille::rechercheDynamic(QString SearchName){
    QSqlQueryModel *model = new QSqlQueryModel;
    model->setQuery("SELECT * FROM FAMILLES WHERE nom LIKE '"+SearchName+"%'");
